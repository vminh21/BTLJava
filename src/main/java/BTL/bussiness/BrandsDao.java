...
public class BrandsDao implements Dao<Brands> {
    ...
    public Optional<Brands> get(int id) {
        ...
        try (Connection conn = myConnection.getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) return Optional.of(new Brands(rs.getInt("brand_id"), rs.getString("name"), rs.getString("origin")));
            }
        } catch (Exception e) { e.printStackTrace(); }
    ...
    @Override public boolean delete(Brands t) { return delete(t.getBrandId()); }
